...
function fdPdf(res, data) {
  ...
  const safeReceiptNo = data.accountNo.replace(/\//g, '-'); // replace slashes
...
const rightFields = [
  ...
  const widths = [75, 85, 45, 45, 80, 85, 100];
  const rowHeight = 32;
...
// ---- Row 3: Nominee Address + Guardian Address
const nomineeAddressY = nomineeRowY + rowGap;

doc.font('Times-Roman')
   .text("Nominee Address :", leftColX, nomineeAddressY);
doc.font('Times-Roman')
...
doc.font('Times-Roman')
   .text("Guardian Address :", rightColX, nomineeAddressY);
doc.font('Times-Roman')
   .text(data.guardianAddress || '-', rightColX + labelWidth1 + textOffset, nomineeAddressY, {
       width: 120,
       align: 'left'
   });
...
doc.font('Times-Roman')
  ...
  function drawFooter(doc) {
    ...
    try { doc.image(stampPath, 70, footerTop, { width: 130 }); } catch (_) {}
    try { doc.image(signPath, pageWidth - 220, footerTop, { width: 130 }); } catch (_) {}

    const textY = footerTop + 90;
    doc.fontSize(12).font('Times-Bold')
      .text('RPD – For Safe, Steady, Stable Growths', 60, textY);
    doc.fontSize(12).font('Times-Bold')
      .text('(Authorised Signatory)', pageWidth - 210, textY);
  }
